...
func (sc *SSOClient) GetLegalEntityByMB(ctx context.Context, mb, token string) (data.LegalEntity, error) {
	var timeout time.Duration
	deadline, reqHasDeadline := ctx.Deadline()
	if reqHasDeadline {
		timeout = time.Until(deadline)
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, sc.address+"/user/mb/"+mb, nil)
	if err != nil {
		return data.LegalEntity{}, err
	}
	...
	resp, err := sc.client.Do(req)
	if err != nil {
		return data.LegalEntity{}, handleHttpReqErr(err, sc.address+"/user/mb/"+mb, http.MethodGet, timeout)
	}
